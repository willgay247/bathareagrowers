...
const AdminWildlifeGardeningPage = () => {
  ...
  const fetch_ = async () => {
    const { data } = await supabase.from("wildlife_gardening_entries").select("*").order("display_order");
    setItems(data ?? []); setLoading(false);
  };
  useEffect(() => { fetch_(); }, []);

  const toggleHidden = async (id: string, hidden: boolean) => {
    await supabase.from("wildlife_gardening_entries").update({ hidden }).eq("id", id);
    setItems((p) => p.map((i) => (i.id === id ? { ...i, hidden } : i)));
  };
  const handleDelete = async () => {
    if (!deleteId) return;
    await supabase.from("wildlife_gardening_entries").delete().eq("id", deleteId);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-[32px] font-bold" style={{ fontFamily: "'Readex Pro', sans-serif", color: "#1E1E1E" }}>Wildlife Gardening</h1>
        {canAddInSection("wildlife_gardening") && (
      ...
      <div className="overflow-x-auto rounded-lg border border-border">
        <table className="w-full text-sm" style={{ fontFamily: "'Readex Pro', sans-serif" }}>
          <thead><tr className="border-b bg-muted/50">
            ...
            {canToggleHidden("wildlife_gardening") && <th className="px-4 py-3 text-center font-semibold">Hidden</th>}
            <th className="px-4 py-3 text-right font-semibold">Actions</th>
          </tr></thead>
          <tbody>
            {items.map((i) => {
              const canEdit = canEditRecord("wildlife_gardening", (i as any).created_by);
              return (
                <tr key={i.id} className="border-b hover:bg-muted/30">
                  ...
                  {canToggleHidden("wildlife_gardening") && (
                  ...
                  <td className="px-4 py-3 text-right space-x-2">
                    ...
                    {canDeleteInSection("wildlife_gardening") && <button onClick={() => setDeleteId(i.id)} className="text-destructive hover:underline text-sm">Delete</button>}
                  </td>
                </tr>
              );
            })}
      ...
      {modalOpen && <SimpleFormModal title="Wildlife Gardening" table="wildlife_gardening_entries" storagePath="wildlife" imageField="image_url" fields={FIELDS} record={editing} onClose={() => { setModalOpen(false); setEditing(null); }} onSaved={() => { setModalOpen(false); setEditing(null); fetch_(); }} createdBy={userId} />}
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete Entry</AlertDialogTitle><AlertDialogDescription>Are you sure? This cannot be undone.</AlertDialogDescription></AlertDialogHeader>
