...
const SupportedGardeningPage = () => {
  ...
  useEffect(() => {
    supabase.from("supported_gardening").select("*").eq("hidden", false).order("display_order").then(({ data }) => {
  ...
  return (
    <main>
      ...
      <section className="relative flex h-[40vh] md:h-[60vh] w-full items-center justify-center overflow-hidden">
        <img src={imageUrl("IMG_3558-200x200.jpeg", { width: 1200, quality: 75 })} alt="Supported gardening in Bath" className="absolute inset-0 w-full h-full object-cover" loading="eager" decoding="async" />
      ...
      <section className="w-full py-10 md:py-20 px-4 bg-background">
        {loading ? (
          ...
          <p className="text-center text-muted-foreground py-12">No entries available.</p>
        ) : (
          <div className="mx-auto max-w-[1100px] grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((p) => (
              <div key={p.id} className="flex flex-col">
                ...
                <h3 className="mt-4 text-[22px] font-bold">{p.name}</h3>
                {p.description && <p className="mt-2 text-[15px] leading-relaxed text-foreground">{p.description}</p>}
                {p.link && <a href={p.link} target="_blank" rel="noreferrer" className="mt-3 text-[15px] font-semibold text-accent hover:underline">More details..</a>}
              </div>
            ))}
          </div>
        )}
